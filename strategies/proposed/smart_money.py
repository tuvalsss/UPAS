...
def _fetch_leaderboard() -> list[dict]:
    """Return list of top wallets above _MIN_PNL. Caches for _LB_REFRESH_SEC."""
    now = time.time()
    if now - _cache["ts"] < _LB_REFRESH_SEC and _cache["wallets"]:
    ...
    try:
        ...
        logger.warning("smart_money.leaderboard_err", extra={"error": str(e)})
        return _cache["wallets"]  # use stale if fetch fails

    wallets = [
        {
            "address": w.get("proxyWallet", "").lower(),
            "name": w.get("userName", ""),
            "pnl": float(w.get("pnl", 0) or 0),
            "vol": float(w.get("vol", 0) or 0),
            "rank": int(w.get("rank", 999)),
        }
        for w in data
        if w.get("proxyWallet") and float(w.get("pnl", 0) or 0) >= _MIN_PNL
    ]
    _cache["wallets"] = wallets
    _cache["ts"] = now
    logger.info("smart_money.leaderboard_refreshed", extra={"n": len(wallets)})
    return wallets
